...
class ConfigParser:
    def __init__(self, args, options='', timestamp=True, test=False, eval_mode=None):
        ...
        if args.resume is None:
            ...
            if eval_mode == "epic":
                resume_cfg_fname = Path('configs/epic.json')
            if eval_mode == "charades":
                resume_cfg_fname = Path('configs/charades.json')
            if eval_mode == "nlq":
                resume_cfg_fname = Path('configs/nlq.json')
            if eval_mode == "mq":
                resume_cfg_fname = Path('configs/mq.json')

...
class ClipFeatureExtractor:
    ...
    def encode_text(self, text_list, bsz=60):
        ...
        for i in range(n_text):
            n_text_per_video = len(text_list[i])
            text_features_per_video = []
            text_eot_features_per_video = []
            for h in range(n_text_per_video):
                # indivdual sampling
                encoded_texts = self.tokenizer(text_list[i][h], context_length=77).to(self.device)
                ...
                for j, valid_len in enumerate(valid_lengths):
                    text_eot_features_per_video.append(output["pooler_output"][j])
                    text_features_per_video.append(batch_last_hidden_states[j, 1:valid_len-1])
            text_features.append(text_features_per_video)
            text_eot_features.append(text_eot_features_per_video)

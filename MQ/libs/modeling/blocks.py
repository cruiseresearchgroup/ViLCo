...
class AdapterMixin:
    ...
    def adapt_module(self, name, input, mask, **kwargs):
        name = normalize_name(name)
        ...
        if hasattr(self, "adapters") and name in self.adapters:
            return self.adapters[name](module, input, mask, **kwargs)
        
        return module(input, mask, **kwargs)


class MaskedConv1D(nn.Module, AdapterMixin):
    """
    ...
    def augment_classification(self, num_new_classes, device):
        feature_dim = self.conv.in_channels
        out_class = self.conv.out_channels
        out_dim = out_class + num_new_classes
        weight = self.conv.weight.data
        bias = self.conv.bias.data
        kernel_size = 3
        new_cls_head = nn.Conv1d(feature_dim, out_dim, kernel_size, stride=1, padding=kernel_size // 2, dilation=1, groups=1, bias=True, padding_mode='zeros').to(device)
        prior_prob = 0.01
        bias_value = -(math.log((1 - prior_prob) / prior_prob))
        torch.nn.init.constant_(new_cls_head.bias, bias_value)
        
        old_cls_head = copy.deepcopy(new_cls_head)
        
        new_cls_head.weight.data[:out_class] = weight[:out_class]
        new_cls_head.bias.data[:out_class] = bias[:out_class]
        # new_cls_head.weight.requires_grad = True
        # new_cls_head.bias.requires_grad = True
        
        self.conv = new_cls_head
...
class MaskedMHA(nn.Module, AdapterMixin):
    """
    ...
    def forward(self, x, mask, encoder_hidden_states=None, encoder_attention_mask=None):
        ...
        att = att.masked_fill(torch.logical_not(attn_mask[:, None, None, :]), float('-inf'))
        # softmax attn
        att = F.softmax(att, dim=-1)
        att = self.attn_drop(att)
        # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
        out = att @ (v * attn_mask[:, None, :, None].to(v.dtype))
...
class MaskedMHCA(nn.Module, AdapterMixin):
    """
    ...
    def forward(self, x, mask):
        ...
        # if T == 1024 or T == 512:
        #     x = torch.stack(x.split(256, dim=-1), dim=-1)
        #     n_chunk = x.shape[-1]
        #     x = x.view(B * n_chunk, C, -1)
        #     mask = torch.stack(mask.split(256, dim=-1), dim=-1)
        #     mask = mask.view(B * n_chunk, 1, -1)
        ...
        out = self.proj_drop(proj_out) * qx_mask.to(out.dtype)

        # if T == 1024 or T == 512:
        #     _, d, len = out.shape
        #     out = out.view(-1, d, len * n_chunk)
        #     qx_mask = qx_mask.view(B, -1, len * n_chunk)
        return out, qx_mask

class ChannelAttention(nn.Module, AdapterMixin):
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(x)
        qkv = qkv.reshape(B, T, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
...
class ChannelBlock(nn.Module, AdapterMixin):
...
class TransformerBlock(nn.Module, AdapterMixin):
    """
    ...
    def __init__(
        ...
        self.channel_attn = ChannelBlock(n_embd, n_head, drop_path=path_pdrop)
    ...
    def forward(self, x, mask, cross_y=None, cross_y_mask=None, pos_embd=None):
        ...
        out, out_mask = self.adapt_module("attn", self.ln1(x), mask) #self.attn(self.ln1(x), mask)
        ...
        # out2 = self.channel_attn(self.ln1(x))
        # out2 = self.channel_attn_down_sample(out2) * out_mask_float

        # out = self.t_c_alpha * out + (1 - self.t_c_alpha) * out2
        # +==================use channel attention
...
class SGPBlock(nn.Module, AdapterMixin):
    """
...
class LocalMaskedMHCA(nn.Module, AdapterMixin):
    """
...
class ConvBlock(nn.Module, AdapterMixin):
    """
    A simple conv block similar to the basic block used in ResNet
    """
    def __init__(
    ...
    def forward(self, x, mask, pos_embd=None, downsample=True):
        identity = x

...
class ANETdetection(object):
    ...
    def _import_ground_truth(self, ground_truth_filename):
        ...
        if self.use_cl:
            ...
            for i in range(num_tasks):
                ...
                sub_activity_index, sub_cidx = {}, 0
                for video in sub_data['dict_db']:
                    if video['id'] in self.blocked_videos:
                        continue
                    for idx, label in enumerate(video['labels']):
                        if label not in sub_activity_index:
                            sub_activity_index[label] = sub_cidx
                            sub_cidx += 1
                        video_lst.append(video['id'])
                        t_start_lst.append(float(video['segments'][idx][0]))
                        t_end_lst.append(float(video['segments'][idx][1]))
                        label_lst.append(sub_activity_index[label])
                sub_ground_truth = pd.DataFrame({'video-id': video_lst,
                                    't-start': t_start_lst,
                                    't-end': t_end_lst,
                                    'label': label_lst})
    ...
    def wrapper_compute_average_precision(self, current_task_id=None):
    ...
    def evaluate(self, current_task_id=None):
        ...
        method.
        """
        self.ap = self.wrapper_compute_average_precision(current_task_id=current_task_id)

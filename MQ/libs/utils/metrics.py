...
def load_gt_seg_from_json(json_file, split=None, label='label_id', label_offset=0, debug_video_id=None, use_cl=False):
    if use_cl:
        data = pkl.load(open(json_file, 'rb'))
        data = data['val']
        ...
        for i in range(num_tasks):
            ...
            for video in sub_data['dict_db']:
                for idx, label in enumerate(video['labels']):
                    video_lst.append(video['id'])
                    t_start_lst.append(float(video['segments'][idx][0]))
                    t_end_lst.append(float(video['segments'][idx][1]))
                    label_lst.append(label)
            gt_base = pd.DataFrame({'video-id': video_lst,
                                't-start': t_start_lst,
                                't-end': t_end_lst,
                                'label': label_lst})
            sub_activity_index = {j: i for i, j in enumerate(sorted(gt_base['label'].unique()))}
            gt_base['label']=gt_base['label'].replace(sub_activity_index)
            ground_truth.append(gt_base)
            activity_index.append(sub_activity_index)
        return ground_truth, activity_index
    else:
        # load json file
        with open(json_file, "r", encoding="utf8") as f:
            json_db = json.load(f)
        if 'database' in json_db.keys():
            json_db = json_db['database']

        vids, starts, stops, labels = [], [], [], []
        for k, v in json_db.items():
            if debug_video_id is not None:
                if v['clip_id'] != debug_video_id[-1]:
                    continue
            ...
            for event in ants:
                # ================== test filter long&short action =========
                # if event['segment'][1] - event['segment'][0] > 30:
                #     idx += 1   
                #     continue
                # ================== test filter long&short action =========
        ...
        activity_index = {j: i for i, j in enumerate(sorted(gt_base['label'].unique()))}
        gt_base['label']=gt_base['label'].replace(activity_index)

        return gt_base, activity_index
...
class ANETdetection(object):
    ...
    def __init__(
        ...
        self.split = split
        ground_truth, activity_index = load_gt_seg_from_json(
            ant_file, split=self.split, label=label, label_offset=label_offset, debug_video_id=debug_video_id, use_cl=self.use_cl)
    ...
    def wrapper_compute_average_precision(self, preds, current_task_id=None):
        """Computes average precision for each class in the subset.
        """

        # Adaptation to query faster
        if self.use_cl:
            ground_truth = self.ground_truth[current_task_id]
            activity_index = self.activity_index[current_task_id]
    ...
    def evaluate(self, preds, current_task_id=None, verbose=True):
        ...
        # compute mAP
        self.ap = self.wrapper_compute_average_precision(preds, current_task_id)
        ...
        # return the results
        return mAP, average_mAP, self.tiou_thresholds

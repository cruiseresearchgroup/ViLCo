...
def train_epoch(config, loader, model, optimizer, schedular, scaler, epoch, output_dir, device, rank, wandb_run=None, ddp=True, cl_name=None, reg_lambda=0., current_task_id=0):
    ...
    for batch_idx, sample in enumerate(loader):
        ...
        if config.train.use_query_roi and 'query_frame' in sample.keys():
            preds = model(clips, 
                          ...
                          training=True, fix_backbone=config.model.fix_backbone, task_id=current_task_id)
        else:
        ...
        if cl_name == 'ewc':
            total_loss = get_regularized_loss(total_loss, model, reg_lambda)
            total_loss.backward()
        elif cl_name == 'mas':
            total_loss = get_mas_regularized_loss(total_loss, model, reg_lambda)
            total_loss.backward()
        else:
            total_loss.backward()
        ...
        if model.module.use_adapt:
            model.module.post_train_step()
...
def validate_cl(config, val_qilDatasetList, model, epoch, current_task_id, output_dir, device, rank, wandb_run=None, ddp=True):
    model.eval()
    metrics = {}
    
    total_iou = AverageMeter()
    total_prob = AverageMeter()
        ...
        for n_task, (val_loader, num_queries) in enumerate(val_loaders_list):
            for batch_idx, sample in enumerate(val_loader):
                ...
                try:
                    ...
                    print(metrics, batch_idx, len(val_loader), len(val_loader))
            ...
            total_iou.update(torch.tensor(metrics['iou']).mean().item(), 1)
            total_prob.update(torch.tensor(metrics['prob_accuracy']).mean().item(), 1)
    ...
    return total_iou.avg, total_prob.avg

def final_validate(config, val_qilDatasetList, model, current_task_id, output_dir, device, rank, wandb_run=None, ddp=True, list_val_iou_ii={'val': []}):
    model.eval()
    metrics = {}
    ...
    with torch.no_grad():
        for n_task, (val_loader, num_queries) in enumerate(val_loaders_list):
            ...
            if n_task == current_task_id:
                list_val_iou_ii['val'].append(metrics['iou'])
            elif n_task < current_task_id:
                forgetting = list_val_iou_ii['val'][n_task] - metrics['iou']
                BWF.update(forgetting, 1)
    ...
    if rank == 0:
        ...
        print(wandb_log)
        # wandb_run.log(wandb_log)
    print("Total_iou_Per_task", total_iou.avg)
    print("Total_prob_Per_task", total_prob.avg)
    print("Total_BWF_Per_task", BWF.avg)
    return total_iou.avg, total_prob.avg, BWF.avg

...
import itertools
import argparse
from dataset.base_dataset import QueryVideoDataset, QueryVideoCLDataset
...
def get_dataset(config, split='train', comp_data=None, use_narration=False, narration_feat_folder=None):
    if split == 'train':
    ...
    elif dataset_name == 'ego4d_vq2d_cl':
        dataset = QueryVideoCLDataset(
            dataset_name=dataset_name,
            query_params=query_params,
            clip_params=clip_params,
            split=split,
            clip_reader=clip_reader,
            current_task_data=comp_data,
            use_narration=use_narration,
            narration_feat_folder=narration_feat_folder,
        )
    return dataset

...
class QueryVideoDataset(Dataset):
    def __init__(self,
        ...
        self.video_dir = '/shared/homes/13045962/Data/ego4d_data/v2/full_scale'

        self.split = split
        if self.split == 'train':
            self.clip_dir = os.path.join(self.clip_dir, "clips_train")
        else:
            self.clip_dir = os.path.join(self.clip_dir, "clips")
    ...
    def _load_metadata(self):
        anno_processed_path = os.path.join('./data', 'vq_{}.json'.format(self.split))
    ...
    def _get_query_path(self, sample):
        ...
        image_path = os.path.join(self.data_dir, 'images', "{}/frame_{:07d}.jpg".format(clip_name, image_name + 1))
        return image_path
...
def read_frames_decord_balance(video_path, num_frames, frame_interval, sample, sampling='rand'):
    ...
    if 'clip_fps' not in sample.keys():
        gt_fps = 5
    else:
...
class QueryVideoCLDataset(Dataset):
    def __init__(self,
                 dataset_name,
                 query_params,
                 clip_params,
        ...
        self.use_naraation = use_narration
        ...
        self.narration_feat_folder = narration_feat_folder
        if self.split  == 'train' and self.use_narration:
    ...
    def _load_metadata(self):
        ...
        self.annotations, n_samples, n_samples_valid = [], 0, 0
        for task_idx, dict_db in self.data.items():
            samples = dict_db['dict_db']
            for sample in samples:
                ...
                curr_anno = {
                    "metadata": {
                        "video_uid": sample["video_id"],
                        # "video_start_sec": sample["video_start_sec"],
                        # "video_end_sec": sample["video_end_sec"],
                        # "clip_fps": sample["clip_fps"],
                    },
                ...
                query_path = self._get_query_path(curr_anno)
                if sample["clip_id"] == '859ed253-d752-4f1b-adc3-c76599117d6e':
                    print(query_path)
                if os.path.isfile(query_path):
                    self.annotations.append(curr_anno)
                    n_samples_valid += 1
                elif self.split == 'train' and sample["clip_id"] == '859ed253-d752-4f1b-adc3-c76599117d6e':
                    print(query_path, curr_anno['clip_uid'], curr_anno['visual_crop']['frame_number'], curr_anno['visual_crop'])
                n_samples += 1
        print('Find {} data samples, {} valid (query path exist)'.format(n_samples, n_samples_valid))
        # with open(anno_processed_path, 'w') as ff:
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train' and self.use_narration:
            results['narration_feats'] = narration_feat.permute(1, 0)
            results['narration_mask'] = narration_mask
        return results
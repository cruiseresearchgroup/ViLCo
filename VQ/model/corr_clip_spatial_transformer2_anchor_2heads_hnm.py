...
class ClipMatcher(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.use_adapt = config['cl']['use_adapt']
        # l2p
        prompt_pool = config['cl']['prompt_pool']
        prompt_length = config['cl']['length']
        top_k = config['cl']['topk']
        pool_size = config['cl']['pool_size']
        if prompt_pool:
            embed_dim = config['cl']['embed_dim']
        ...
        # ssl
        self.narration_ssl = config['cl']["narration_ssl"]
        self.narration_dim = config['cl']["narration_dim"]
        if self.narration_ssl:
            feature_dim = 384
            self.narration_encoder = nn.Linear(config['cl']['narration_dim'], feature_dim)
            self.memory_bank = MemoryBank(config['cl']['memory_size'], feature_dim)
        self.ssl_factor = config['cl']["ssl_factor"]
        ...
        self.use_adapt = config['cl']['use_adapt']
        if self.use_adapt:
            self.adapt_blocks = config['cl']['adapt_blocks']
    ...
    def forward(self, clip, query, query_frame_bbox=None, training=False, fix_backbone=True, task_id=-1):
        '''
